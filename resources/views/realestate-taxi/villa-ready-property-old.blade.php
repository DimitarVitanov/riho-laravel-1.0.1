...
<div class="wrap">
  <section class="hero">
    <span class="eyebrow">{{ $property->hero_eyebrow ?? 'Exclusive property offered by our agency' }}</span>
    <h1>{{ $property->title }}</h1>
    <p class="hero-desc">{{ $property->intro }}</p>
    <div class="property-meta">
      @if($property->hero_chips && count($property->hero_chips))
        @foreach($property->hero_chips as $chip)
  ...
  <div class="layout">
    <main>
      ...
      <section class="card pad">
        ...
        <div class="grid2">
          <div class="mini-card"><b>Location</b><p>{{ $property->location_card_text ?? $property->location_description ?? $property->location }}</p></div>
          <div class="mini-card"><b>Project</b><p>{{ $property->project_card_text ?? $property->structure ?? 'Modern residential development with swimming pools, private outdoor areas, garages and storage.' }}</p></div>
          <div class="mini-card"><b>Use Options</b><p>{{ $property->use_options_card_text ?? $property->use_options ?? 'Private living, holidays, tourist rental income, long-term ownership or future resale.' }}</p></div>
          <div class="mini-card"><b>Optional Management</b><p>{{ $property->management_card_text ?? $property->management_service ?? 'Booking management, guest communication, cleaning, maintenance, rental pricing and local property supervision.' }}</p></div>
        </div>
      </section>

      @if($property->building_1_description || $property->building_2_description || $property->building_3_description || $property->building_4_description)
      <section class="card pad">
        <div class="title"><div><h2>Building Information</h2><p class="sub">The admin can enter and update every building, floor, unit, size and price manually.</p></div></div>
        <div class="grid2">
          @if($property->building_1_description)
          <div class="mini-card"><b>Building 1</b><p>{{ $property->building_1_description }}</p></div>
          @endif
          @if($property->building_2_description)
          <div class="mini-card"><b>Building 2</b><p>{{ $property->building_2_description }}</p></div>
          @endif
          @if($property->building_3_description)
          <div class="mini-card"><b>Building 3</b><p>{{ $property->building_3_description }}</p></div>
          @endif
          @if($property->building_4_description)
          <div class="mini-card"><b>Building 4</b><p>{{ $property->building_4_description }}</p></div>
          @endif
        </div>
      </section>
      @endif
      ...
      <section class="card pad agency-contact" id="contact">
        <div class="title"><div><h2>{{ $property->contact_form_title ?? 'Contact Our Agency About This Property' }}</h2><p class="sub">{{ $property->contact_form_subtitle ?? 'Your enquiry goes to the real estate agency presenting this property.' }}</p></div></div>
    ...
    <aside class="sidebar">
      <div class="price-panel">
        <small>{{ $property->sidebar_price_label ?? 'NET PRICE' }}</small>
        <div class="big">{{ $property->sidebar_price_value ?? $property->price_display }}</div>
        <p>{{ $property->sidebar_price_note ?? 'Final unit price depends on floor, size and availability.' }}</p>
        <a class="btn" href="#contact" style="width:100%">Ask Our Agency</a>
      </div>
      <div class="card sidebar-box">
        <div class="head">Key Facts</div>
        <div class="sidebar-list">
          @if($property->key_facts && count($property->key_facts))
            @foreach($property->key_facts as $fact)
            <div class="sidebar-row"><span class="ico">{{ $fact['icon'] ?? '•' }}</span><span>{{ $fact['text'] ?? $fact }}</span></div>
            @endforeach
          @else
