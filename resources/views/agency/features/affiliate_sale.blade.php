...
@section('title', 'Affiliate Sale')
@section('breadcrumb-title')<h3>Affiliate Sale</h3>@endsection
@section('breadcrumb-items')
    <li class="breadcrumb-item"><a href="{{ route('agency.dashboard') }}">Agency Panel</a></li>
    <li class="breadcrumb-item active">Affiliate Sale</li>
@endsection
@section('content')
<style>
...
.vrc-note{background:#fef3c7;border:1px solid #fcd34d;border-radius:8px;padding:12px;font-size:12px;color:#92400e;margin-bottom:16px}
</style>
...
<div class="container-fluid">
    ...
    <div class="vrc-note">
        <strong>⚠️ Affiliate Sale is currently OFF.</strong> When disabled, the Invisible Lead Magnet and Villa Ready property pages will not be uploaded to your server. Turn it ON to start earning commissions.
    </div>
    @endif
        ...
        <div class="col-xl-3 col-md-6">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="vrc-summary">{{ number_format($stats['total_visits'] ?? 0) }}</div>
                </div>
            </div>
        </div>
        <div class="col-xl-3 col-md-6">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="vrc-summary">{{ number_format($stats['viewed'] ?? 0) }}</div>
                </div>
            </div>
        </div>
        <div class="col-xl-3 col-md-6">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="vrc-summary">{{ number_format($stats['paid_sales'] ?? 0) }}</div>
                </div>
            </div>
        </div>
        <div class="col-xl-3 col-md-6">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="vrc-summary text-success">€{{ number_format($stats['total_commission'] ?? 0, 2) }}</div>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="property-card">
                        ...
                        @elseif($pub->property->images->first())
                        <img src="{{ $pub->property->images->first()->image_url }}" alt="{{ $pub->property->title }}">
                        @else
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <span class="badge bg-light text-dark">{{ $pub->property->price_display ?? 'Price on request' }}</span>
                            <code style="font-size:11px">{{ $pub->affiliate_code }}</code>
                        </div>
                        @if($pub->published_url)
                        <div class="mt-2">
                            <a href="{{ $pub->published_url }}" target="_blank" class="btn btn-sm btn-outline-primary w-100">View Published Page</a>
                        </div>
                        @endif
                    </div>
                </div>
                @empty
                <div class="col-12">
                    <p class="text-muted text-center py-4">No properties published to your website yet. Properties are automatically published when you upload your sitemap.</p>
                </div>
                @endforelse
            </div>
        </div>
    ...
    <div class="card">
        ...
        <div class="card-body">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <strong class="text-success">{{ $referral->formatted_commission ?? '€0.00' }}</strong>
                                @else
            ...
            @if(isset($referrals) && $referrals->hasPages())
            {{ $referrals->links() }}
            @endif
        </div>
    </div>
