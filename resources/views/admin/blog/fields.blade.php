<div class="form theme-form">
    ...
    <div class="col-sm-12">
        ...
        <div class="mb-3">
            <label for="" class="w-100">Content<span class="text-danger">*</span></label>
            ...
            <input type="hidden" name="content" value="{{ isset($blog->content) ? $blog->content : old('content') }}"
                    ...
                    @foreach ($blog->getMedia('images') as $image)
                        <div class="col-xxl-1 col-md-2 col-sm-3 col-auto mb-2">
                            <div class="multi-remove position-relative">
                                <img src="{{ $image->getUrl() }}" alt="Image" class="img-thumbnail img-fix"
                                    width="100%">
                                <a href="" class="dz-remove text-danger"
                                    data-bs-target="#tooltipmodal{{ $image->id }}" data-bs-toggle="modal">Remove</a>
        ...
        <div class="mb-3">
            <div class="col-form-label">Categories
                <select class="js-category-placeholder-multiple" multiple="multiple" name="categories[]">
                    ...
                    @dump($key)
                        <option value="{{ $key }}"
                            {{ (is_array(old('categories')) && in_array($key, old('categories'))) ||
                            (isset($default_categories) && in_array($key, $default_categories))
                                ? 'selected'
                                : '' }}>
                            {{ $value }}
                        </option>
        ...
        <div class="mb-3">
            ...
            <div class="d-flex flex-column-reverse">
                <select class="form-select status-placeholder" name="status">
                    ...
                    <option value="1"
                        @if (isset($blog->status)) @if ('1' == $blog->status) selected @endif @endif
                        @if (old('status') == '1') selected @endif>{{ __('Active') }}</option>
                    <option value="0"
                        @if (isset($blog->status)) @if ('0' == $blog->status) selected @endif @endif
